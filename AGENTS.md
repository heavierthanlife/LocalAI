...
> **文档导航**：`README.md`（入口）· `docs/MANIFEST.md`（目录地图）· `docs/ARCHITECTURE.md`（深度架构）· `CHANGELOG.md`（变更历史）· `docs/DECISIONS.md`（技术决策）· `docs/SECURITY.md`（安全基线）· `docs/USER_MANUAL.md`（运维手册）· 本文件为开发者/Agent 命令与约定指南。
...
# Install deps (full pinned, or minimal unpinned)
pip install -r requirements.txt        # ~156 packages
pip install -r requirements_min.txt    # 精简（无版本锁定）
...
- **不要**提交密钥/私钥/二进制（`cert/key.pem`、`msedgedriver.exe`）。
- **提交钩子**（`core.hooksPath=.githooks`）：`pre-commit` 运行 `scripts/verify_fixes.py`（fix_registry 不变式）+ `repair_kit/check_integrity.py`（蓝图↔清单覆盖）+ `scripts/check_doc_drift.py`（文档计数↔代码）；`post-commit` 仅写 `data/qa_loop/pending.flag`（待运行标记，不自动跑 loop）。
- 旁路：`SKIP_FIX_CHECK=1` / `SKIP_REPAIR_KIT_CHECK=1` / `SKIP_QA_LOOP_HOOK=1`（仅调试，勿常规使用）。
...
| 回归 | `pytest tests/test_regression.py -s` | FIX 回归锁 |
| 批量 | `pytest tests/test_batch_orchestrator.py` | 串通检测/TF-IDF |
| 路由守护 | `pytest tests/test_route_preservation.py` | 路由集合不缩水 |
| 集成 | `python tests/run_tests.py` | 仅导入，无 pytest |
| 全量 | `pytest tests/` | 全部（默认 deselect `db`） |
...
**可选依赖**：`hanlp`（relationship NER）标注在 `requirements.txt` 可选段，缺失时自动走后备（regex），不影响核心功能。
...
- **fallback 链**：`llm_fallback.py` 尚未接线（仅 tests 引用），当前为单供应商直连；接线见 `data/unresolved.yaml`。
...
- **子系统校准**（FIX-015）：quote（`_is_price_like` 上下文过滤/`quote_with_open_price` 开标价注入/risk 幅度加权）· relationship（公司/人员正则黑名单/risk 归一化）。
- **数据门槛（避免误报）**：
...
根目录仅 3 份文档；其余在 `docs/`（平铺）。

| 文件 | 何时更新 |
|---|---|
| `README.md` | 功能入口、快速开始变化 |
| `AGENTS.md` | 开发命令、约定、Gotchas、环境变量、SOP |
| `CHANGELOG.md` | 每次功能/修复（顶部新增，含 regression 标注） |
| `docs/ARCHITECTURE.md` | 架构层变化（新蓝图/新服务/数据流变化） |
| `docs/MANIFEST.md` | 目录结构变化（纯目录地图，无计数） |
| `docs/DECISIONS.md` | 技术选型决策 |
| `docs/SECURITY.md` | 安全发现/修复 |
| `docs/USER_MANUAL.md` | 运维命令/配置/操作变化 |
| `docs/IMPROVEMENTS_SKIPPED.md` | 历史记录（不新增，活跃项进 `data/unresolved.yaml`） |
| `repair_kit/SYSTEM_CHECKLIST.md` | 由 `scripts/check_system.py` 自动更新 |

计数类事实（蓝图/表/服务/JS/供应商/指标）由 `scripts/check_doc_drift.py` 对照 `README.md`/`AGENTS.md`/`docs/{ARCHITECTURE,MANIFEST,USER_MANUAL}.md` 自动校验（pre-commit 运行）。
